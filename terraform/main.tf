...
resource "kubernetes_deployment" "api" {
  metadata { name="distributed-ai-inference-platform" namespace=var.namespace labels={app="distributed-ai-inference-platform"} }
  spec { replicas=var.replicas selector { match_labels={app="distributed-ai-inference-platform"} } template {
    metadata { labels={app="distributed-ai-inference-platform"} }
...
resource "kubernetes_service" "api" {
 metadata {name="distributed-ai-inference-platform" namespace=var.namespace}
 spec { selector={app="distributed-ai-inference-platform"} port {port=80 target_port=8000} }
}
